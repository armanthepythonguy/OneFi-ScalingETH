...
func (ba *FetchingAttributesBuilder) PreparePayloadAttributes(ctx context.Context, l2Parent eth.L2BlockRef, epoch eth.BlockID) (attrs *eth.PayloadAttributes, err error) {
	...
	l1RandomTx, err := L1RandomDepositBytes(seqNumber, l1Info, sysConfig)
	if err != nil {
        	return nil, NewCriticalError(fmt.Errorf("failed to create l1InfoTx: %w", err))
	}
	l1BTCFeedTx, err := L1BTCFeedDepositBytes(seqNumber, l1Info, sysConfig)
	if err != nil {
        	return nil, NewCriticalError(fmt.Errorf("failed to create l1InfoTx: %w", err))
	}
	...
	l1AutomateTx, err := L1AutomateDepositBytes(seqNumber, l1Info, sysConfig)
	if err != nil {
        	return nil, NewCriticalError(fmt.Errorf("failed to create l1InfoTx: %w", err))
	}

	txs := make([]hexutil.Bytes, 0, 5+len(depositTxs))
	txs = append(txs, l1InfoTx)
	txs = append(txs, l1RandomTx)
	txs = append(txs, l1BTCFeedTx)
	txs = append(txs, l1ETHFeedTx)
	txs = append(txs, l1AutomateTx)
